<form id="flight-offer-form" method="POST"  action="{{ route('flight.admin.offers.store') }}" >
    ...
    <div class="card shadow-sm mb-4 border-0">
        <div class="card-header bg-primary text-white">
            <h4 class="mb-0">✈️ {{ __("Recherche de Vols Multiples Aller-Retour") }}</h4>
        </div>

        <div class="card-body">
            <!-- Offres -->
            <h5 class="mb-3 border-bottom pb-2">🛫 {{ __("Offres Aller-Retour") }}</h5>
...
setTodayAsMinDateForAllDateInputs();
        ...
        $('#flight-offer-form').on('submitt', function (e) {
            ...
            // AJAX POST Request
            $.ajax({
                url: "{{ route('flight.admin.offers.store') }}", // Update the route name if needed
                type: 'POST',
                data: formData,
