    ...
    <nav aria-label="breadcrumb" class="mb-4">
        <ol class="breadcrumb bg-white px-3 py-2 rounded shadow-sm">
            <li class="breadcrumb-item">
                <a href="{{ url('/') }}" class="text-decoration-none text-primary">Accueil</a>
            </li>
            <li class="breadcrumb-item">
                <a href="{{ route('offres.index') }}" class="text-decoration-none text-primary">Offres</a>
            </li>
            <li class="breadcrumb-item active" aria-current="page">
                {{ $scraping->hotel_name ?? 'Offre' }}
            </li>
        </ol>
    </nav>
    <div class="row g-4">
                ...
                <div class="hotel-header-modern">
                    <div>
                        <div class="hotel-title-modern">
                            ...
                            {{ $scraping->hotel_name ?? 'Hôtel Inconnu' }}
                            @if($scraping->rating)
                                <span class="badge rating-badge-modern">
                                    ...
                                    <span class="ms-1 fs-6">{{ $scraping->rating }}/10</span>
                                </span>
                            @endif
                        </div>
                        @if($scraping->address)
                        <div class="hotel-address-modern">
                            ...
                            <span>{{ $scraping->address }}</span>
                        </div>
                        @endif
                    </div>
                </div>

                @include('offres.partials._gallery', ['scraping' => $scraping])

                <div class="p-4">
                    <h4 class="section-title">Types de chambres disponibles</h4>
                    @include('offres.partials._room_types', ['types' => $types])
                </div>

                
                <div class="p-4">
                    <h4 class="section-title">Services proposés</h4>
                    @include('offres.partials._services', ['services' => $services])
                </div>
        ...
        <div class="col-lg-3">
            <div class="right-sticky">
                <!-- OWNER BLOCK -->
                <a href="{{ route('user.profile', ['id' => $owner->user_name ?? $owner->id]) }}" class="text-decoration-none">
                    <div class="owner-block">
                        <div class="d-flex align-items-center gap-3">
                            <img src="{{ $owner->getAvatarUrl() }}" alt="Avatar" class="owner-avatar">
                            <div>
                                <div class="owner-name">{{ $owner->getDisplayName() }}</div>
                                <div class="owner-contact">
                                    Membre depuis {{ \Carbon\Carbon::parse($owner->created_at)->translatedFormat('F Y') }}
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="reservation-block p-3 shadow-sm">
                    @include('offres.partials._reservation_form', ['offre' => $offre])
                </div>
            </div>
        </div>
    </div>
