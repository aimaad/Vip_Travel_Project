    ...
    class TourController extends Controller
    {
        ...
        public function index(Request $request)
        {
            ...
            if ($is_ajax) {
                ...
                // Vérifier d'abord s'il existe des tours
                if ($resultCount > 0) {
                    // Ensuite, vérifier si la correspondance est exacte ou non
                    $resultMessage = self::$isExact 
                        ? ($resultCount > 1 ? __(":count tours found", ['count' => $resultCount]) : __(":count tour found", ['count' => $resultCount]))
                        : __('No exact match found. However, here are some similar tours you might like.');
                } else {
                    // Aucun tour n'existe
                    $resultMessage = __('No tours found, and no similar tours are available.');
                }
            
                return $this->sendSuccess([
                    "markers" => $markers,
                    "isExact" => self::$isExact, // Récupère la valeur de $isExact
                    'fragments' => [
                        '.ajax-search-result' => view('Tour::frontend.ajax.search-result' . ($for_map ? '-map' : ''), $data)->render(),
                        '.result-count' => $resultMessage,
                        '.count-string' => $resultCount ? __("Showing :from - :to of :total Tours", ["from" => $list->firstItem(), "to" => $list->lastItem(), "total" => $resultCount]) : ''
                    ]
                ]);
            }
