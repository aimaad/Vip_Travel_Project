...
class Tour extends Bookable
{
    ...
    public function search($request)
    {
        ...
        TourController::$isExact = true; // Initialisation
        ...
        $results = $model_Tour->get();
        // Si aucun résultat exact n'est trouvé, rechercher des résultats similaires basés sur la localisation
    if ($results->isEmpty()) {
        Log::info('No exact matches found, searching for similar tours based on location.');
        TourController::$isExact = false; // Si aucun résultat exact n'est trouvé

        // Réinitialiser la requête pour une recherche basée uniquement sur la localisation
        $model_Tour = parent::query()->select("bravo_tours.*");
        $model_Tour->where("bravo_tours.status", "publish");

        // Appliquer la localisation (comme précédemment)
        ...
        // Réappliquer les autres filtres (prix, catégorie, etc.) aux résultats similaires
        if (!empty($price_range = $request['price_range'] ?? "")) {
            $pri_from = Currency::convertPriceToMain(explode(";", $price_range)[0]);
            $pri_to =  Currency::convertPriceToMain(explode(";", $price_range)[1]);
            $raw_sql_min_max = "( (IFNULL(bravo_tours.sale_price,0) > 0 and bravo_tours.sale_price >= ? ) OR (IFNULL(bravo_tours.sale_price,0) <= 0 and bravo_tours.price >= ?) )
        ...
        // Exécuter la requête pour obtenir les résultats similaires
